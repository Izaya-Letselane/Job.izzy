...
const PriceCard = ({type, price}: Props) => {
  return (
    <div className="bg-gray-100 dark:bg-gray-700 p-10 rounded-lg relative">
        {/*Type check for recommende badge */}
      {type ==='Standard' &&  
      (<div className="absolute top-7 rigt-7 px-6 py-1.5 bg-green-800 text-white rounded-full text-xs">
        Recommended
        </div>
    )}
    {/*Type*/}
    ...
    <div className="mt-8 text-center">
        <button className="w-full py-3 bg-blue-900/15 dark:bg-gray-100 text-blue-700 rounded-full 
        hover:bg-blue-700 hover:text-white transition-all duration-300 cursor-pointer">View Profile</button>
    </div>
    </div>
  )
}
